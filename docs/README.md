# Documentation

## Use the application

- [Run locally and troubleshoot](../HOW-TO-RUN.md)
- [Policy review: plain-English desktop guide](policy-review/README.md)
- [Policy review: CLI and technical reference](policy-review.md)
- [Policy pilot deployment](deployment/POLICY-PILOT.md)
- [Refactor assessment](REFACTOR-ASSESSMENT.md)
- [Architecture decision](adr/001-offline-policy-review.md)
- [Policy pilot implementation and validation](POLICY-IMPLEMENTATION.md)

## Maintain the repository

- [Scripts and their outputs](../tools/README.md)
- [Workspace and corpus data](../data/README.md)
- [Acquire source documents](../sources/README.md)
- [Framework relationships and editions](../sources/framework-notes.md)
- [Add a framework, controls and assessments](../sources/add-framework.md)
- [Run tests and data checks](../tests/README.md)

## Review reports

These reports record the scope and findings of their dated reviews.

- [Assessment methods and commands](assessment-method-review.md)
- [Corpus validation and topic coverage](corpus-revalidation.md)
- [Framework coverage](framework-coverage-review.md)
- [Source acquisition](source-acquisition-review.md)
- [WA PRIS source and scope](wa-pris-framework.md)
- [Repository improvements](repository-review.md)
