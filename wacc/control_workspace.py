"""Control-centred privileged-access pilot; publisher records remain unchanged."""
import csv
import html
import io
import json
...
LIBRARY = json.loads((Path(__file__).resolve().parents[1] / 'data/library/privileged-access.json').read_text(encoding='utf-8'))
...
def matching(query, selected):
    tokens = re.findall(r'\w+', query.lower())
    return [c for c in CONTROLS if mappings(c, selected) and all(t in
            ('privileged access ' + json.dumps({**c, 'mappings': mappings(c, selected)}, ensure_ascii=False)).lower() for t in tokens)]


def url(selected, control=None, query=''):
...
def export_csv(params):
    ...
    for c in matching((params.get('q') or [''])[0], selected):
...
(function(){
 ...
 form.addEventListener('submit',e=>{e.preventDefault();try{localStorage.setItem(key,JSON.stringify(readForm()));dirty=false;state.textContent='Saved in this browser.';}catch(e){state.textContent='Could not save. Download the record to keep it.';}});
 form.addEventListener('input',()=>{dirty=true;state.textContent='Unsaved changes';});
...
def render(corpus, params):
    ...
    listing=''.join('<a class="control-link" href="%s"%s><small>%s</small><strong>%s</strong><small>%d source references</small></a>' %
                    (esc(url(selected,c['id'],query)), ' aria-current="page"' if chosen and chosen['id']==c['id'] else '',c['id'],esc(c['title']),len(mappings(c,selected))) for c in found)
    sidebar='<aside class="sidebar"><form class="filters" action="/library"><label for="library-q">Find a control</label><input id="library-q" type="search" name="q" value="%s" placeholder="Privileged access or ISM-1507"><input type="hidden" name="scope" value="1"><fieldset><legend>Framework scope</legend>%s</fieldset><button>Apply scope and search</button></form><div class="controls"><small>%d controls in scope</small>%s</div><p class="muted">%s</p><a href="%s">Export scoped mappings (CSV)</a></aside>' % (esc(query),checkboxes,len(found),listing,'' if found else 'No pilot controls match this scope and search.',esc(url(selected,query=query).replace('/library?','/library/export.csv?')))
    ...
    if chosen:
        ...
        main='''<article><section class="box"><div class="eyebrow">%s · Privileged access</div><h2>%s</h2><p>%s</p><p class="muted"><strong>Scope:</strong> %s</p>%s<div class="risk"><strong>Business risk</strong><br>%s</div><nav><a href="#assessment">Assessment</a><a href="#record">Evidence and result</a><a href="#sources">Source requirements</a></nav></section>
...
<section class="box" id="sources"><h3>Source requirements · %d references in scope</h3><p class="muted">These locally reviewed connections describe overlap, not compliance. An effective control does not automatically satisfy every linked requirement.</p>%s</section><section class="box"><h3>Related controls</h3><p>%s</p></section></article><script type="application/json" id="control-context">%s</script>''' % (
            c['id'],esc(c['title']),esc(c['statement']),esc(c['scope']),'<p class="muted">This control has no references in the selected framework scope.</p>' if not refs else '',esc(c['risk']),esc(c['examine']),esc(c['interview']),esc(c['test']),esc(c['expected']),len(refs),''.join(rows) or '<p>No source references selected.</p>',related,json.dumps(context,ensure_ascii=False).replace('<','\\u003c'))
    return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>WACC · Control workspace</title><style>%s</style></head><body><header><div><div class="eyebrow">WA Control Crosswalk</div><h1>Control workspace</h1></div><a href="/?q=privileged+access">Browse source corpus →</a></header><p class="pilot">Privileged-access pilot · Five controls with reviewed source connections. Other subjects remain available in the source browser.</p><div class="workspace">%s<main>%s</main></div><script>%s</script></body></html>' % (STYLE,sidebar,main,SCRIPT)
