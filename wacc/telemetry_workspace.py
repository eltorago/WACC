...
def strip(report):
    c = counts(report)
    label = f'{c["support"]} supported, {c["gap"]} gaps, {c["review"]} need review, {c["none"]} without evidence'
...
def _finding(f, run, token, stale):
    ...
    for e in f['evidence'][:20]:
        row = e['row']; identity = row.get('DeviceName') or row.get('UserPrincipalName') or row.get('DeviceId') or row.get('UserId')
        label = f'{e["time"]} · {identity} · {row.get("ActionType", e["table"])}'
...
<ol class="validation-steps"><li>Declare the devices, users, apps and evidence dates in <code>manifest.json</code>.</li><li>Export the six Sentinel tables as JSON lines or CSV.</li><li>Import, inspect each finding and record a review decision.</li></ol>
...
<section class="box"><h3>Azure and Sentinel</h3><p>Read existing ADLS Gen2 exports using the <code>wacc telemetry --adls</code> command. For Sentinel’s managed data lake, export CSV from the KQL query editor.</p><p><a href="/assessments/validation/guide">Setup and export guide</a></p><p><a href="/assessments/validation/queries">Download KQL export queries</a></p><p><a href="{SOURCES['wa']}">WA SOC onboarding guidance</a></p></section></aside>'''
    if not record:
        ...
        if not run:
            ...
            table_rows = ''.join(f'<tr><th>{t}</th><td>{m.get("expected_rows",{}).get(t,"Not supplied")}</td><td>{q["table_rows"][t]}</td><td>{q["scope_rows"].get(t,0)}</td></tr>' for t in TABLES)
