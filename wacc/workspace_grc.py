...
def render(control_id, corpus, selected):
    ...
    status='' if verified else '<p role="status">Publisher document match needs revalidation: source missing or changed.</p>'
    ...
    return '<div class="grc-evidence"><h4>Documents to examine</h4><p>%s</p><ul>%s</ul><h4>What to check</h4><p>%s</p>%s</div>'%(
        link,''.join('<li>%s</li>'%esc(d) for d in evidence['documents']),esc(evidence['application']),status)
